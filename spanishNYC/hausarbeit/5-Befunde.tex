...
Zunächst haben wir es hier mit Lebensmitteln in Rezepten oder Diätprogrammen zu tun, die oft auf Englisch, oder auf Spanisch mit der englischen Übersetzung in Klammern erscheinen.
...
Verwandt mit der vorherigen Kategorie wurde auch die Gruppe der Switches, die unter ``Gesundheit''  angegliedert werden können, identifiziert.
...
Beispiele hieraus umfassen allgemeine Technologiebegriffe wie ``chip'', ``laptop'', ``upgrade'', ``software'';
...
wiederum andere hatten mit Makeup zu tun ``smoky eye'', ``cat eye'', ganz prominent darunter waren Bezeichnungen für Farbtönen: ``nude'' ``coral reef'', ``snow shadow'', ``cherry red''.
...
Ferner wurden auch einige allgemeine Entlehnungen aus dem Englischen identifiziert.
...
Die Konzepte/Gegenstände wurden von da aus in anderen Teilen der Welt übernommen, und da sie vorher nicht existiert haben, werden in den allermeisten Fällen auch gleich ihre Bezeichnungen mit entlehnt.
...
% n-th order indexicality: Produkte verkaufen
...
Unter anderem werden nämlich einfach Produkte selber ins Englische geswitched: ``baby carrots'', ``Dirty Martini'' oder auch unzählige Markenkosmetikartikel wie ``Maybeline Baby Lips Lip Balm en Cherry Me'', ``Maybeline Fit Me Foundation'', ``Daily Moisture Shampoo \& Conditioner'' u.s.w.
% TODO check McCracken für weitere stilistische Devices
...
%TODO mehr Produkt/Dienstleistungsbeispiele!
\end{comment}
...
``Look'', also das äußere Aussehen, wird besonders bei Frauen oft mit Erfolg (oder seinem Fehlen) gleichgestellt, wie die britische Journalistin und Autorin Laurie Penny argumentiert:
...
%Diese Kritik wird auch von Mary Talbot aufgegriffen und bezüglich ``Frauen''-Zeitschriften weiter ausgeführt:
...
Wiederum wird Spanisch mit Marginalisierung, Armut, Rückstand, fehlender Bildung und dem nicht-weißen Anderen assoziiert~\cite{Zentella07}.
